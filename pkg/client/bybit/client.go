...
func getV5Signature(timestamp int64, key string, queryString string, secret string) string {
...
func getV5SignatureForBody(timestamp int64, key string, body []byte, secret string) string {
	val := strconv.FormatInt(timestamp, 10) + key
	val = val + string(body)
	h := hmac.New(sha256.New, []byte(secret))
	h.Write([]byte(val))
